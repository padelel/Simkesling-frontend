...
const FormPengajuanTransporter: React.FC = () => {
  ...
  const getDatesHere = async () => {
    ...
    for (let index = 0; index < lengthdate; index++) {
      ...
      if (pengajuanTransporterStore.files) {
        ...
        // console.log(val);
        let tmpdateawal = [dayjs(val.tgl_mulai)];
        let tmpdateakhir = [dayjs(val.tgl_akhir)];
        // console.log(a);
        arrawal.push(tmpdateawal);
        arrakhir.push(tmpdateakhir);
      }
    }
    console.log(arrawal);
    setDateRangeAwal(arrawal);
    setDateRangeAkhir(arrakhir);
    console.log(dateRangeAwal);
    const listMouDynamic = formInstance.getFieldValue("listMouDynamic");
    console.log(listMouDynamic);
  ...
  useLayoutEffect(() => {
    if (pengajuanTransporterStore.id_transporter_tmp == null || pengajuanTransporterStore.id_transporter_tmp == 0) {
